...
use App\Jobs\DataCollection\AWD\ListInboundShipmentsJob;
use App\TE\HelperClasses\ETLHelper;
use Illuminate\Console\Command;

class ListInboundShipmentsCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'download:awd_inbound_shipments
        ...
        foreach ($profile_types as $profile_type) {
            ...
            foreach ($profilesForDataDownload as $profile) {
                ...
                if (in_array($profile->id, $skip_profile)) {
                    $this->warn('Skipping [download:awd_inbound_shipments] command for ' . $profile->profile_type . ' and profile ' . $profile->id);
                    continue;
                }

                $this->info('Running [download:awd_inbound_shipments] command for type ' . $profile->profile_type . ' and profile ' . $profile->id);
                ...
                $dateRanges = ETLHelper::getApiPatterns($profile_info['profile_type'], AWD_INBOUND_SHIPMENT, $backfill, $customDateRange, $reportRange, "ASC");
                ...
                foreach ($dateRanges as $dateRange) {
                    ...
                    $job = new ListInboundShipmentsJob($profile_info, $updatedAfter, $updatedBefore);
