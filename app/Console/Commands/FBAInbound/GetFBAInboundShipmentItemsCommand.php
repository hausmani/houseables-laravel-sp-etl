...
use App\Jobs\DataCollection\FBAInbound\GetFBAInboundShipmentItemsJob;
use App\Jobs\DataCollection\FBAInbound\GetFBAInboundShipmentsJob;
use App\TE\HelperClasses\ETLHelper;
use Illuminate\Console\Command;

class GetFBAInboundShipmentItemsCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'download:fba_inbound_shipment_items
     ...
     * @var string
     */
    protected $description = 'Command for getting FBA inbound shipment items for profiles from API';
        ...
        foreach ($profile_types as $profile_type) {
            ...
            foreach ($profilesForDataDownload as $profile) {
                ...
                if (in_array($profile->id, $skip_profile)) {
                    $this->warn('Skipping [download:fba_inbound_shipment_items] command for ' . $profile->profile_type . ' and profile ' . $profile->id);
                    continue;
                }

                $this->info('Running [download:fba_inbound_shipment_items] command for type ' . $profile->profile_type . ' and profile ' . $profile->id);
                ...
                $dateRanges = ETLHelper::getApiPatterns($profile_info['profile_type'], FBA_INBOUND_SHIPMENT_ITEM, $backfill, $customDateRange, $reportRange, "ASC");
                ...
                foreach ($dateRanges as $dateRange) {
                    ...
                    $job = new GetFBAInboundShipmentItemsJob($profile_info, $updatedAfter, $updatedBefore);
